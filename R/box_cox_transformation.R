...
#'
#' @param s vector with data
#' @param lambda lambda parameter of box-cox tranformation
#'
#' @return Box-Cox transformation of input vector with lambda parameter
#'
...
#' #'
#' #' @return Loglikehood function
#' loglik.box2 = function(x, lambda) {
#'
#'   x_box = box.cox(abs(qnorm(x)), lambda = lambda)
#'   x_box = (x_box - mean(x_box, na.rm = T)) / sd(x_box, na.rm = T)
#'   x_box = x_box[order(x_box)]
#'
#'   max(abs(pnorm(x_box) - ((row_number(x_box) - 1) / length(x_box[!is.na(x_box)]))),
...
#' #' @return Return the lambda parameter which minimizes the loglikehood function
#' #'
#' lambda.box2 = function(x = x, interval = c(-20,20), tol = 0.00001) {
#'
#'   if (is.data.frame(x)) {
#'     x = lapply(x, function(x) x)
#'   }
...
#' #' DtD.box
#' #'
#' #' @param x Frequency Default Observation vector
#' #' @param lambda (Optional) lambda, if NULL, lambda is optimized
#' #' @param plot Logical, plot qqplots
#' #' @param ks.test Logical, kolmogorov-smirnov test
#' #'
#' #' @return Compute the box-cox transformation based on lambda of FDO serie. Plot and ks.test argument can be set to TRUE in order to asses the normality of the transformed vector
#' #'
#' DtD.box2 = function(x = x, lambda = NULL, plot = F, ks.test = F) {
#'
#'   qqPlot = getFromNamespace("qqPlot", "car")
#'
#'   if (missing("lambda")) {
...
#'                               sd = sd(x, na.rm = T))
#'
#'     kstest$transformed = ks.test(DtD.box$x.box,
#'                                  "pnorm",
#'                                  mean = mean(DtD.box$x.box, na.rm = T),
#'                                  sd = sd(DtD.box$x.box, na.rm = T))
#'
#'     nombre = c("statistic", "p-value")
#'     Dtd = c(kstest$Dtd$statistic %>% as.numeric,
#'             kstest$Dtd$p.value %>% as.numeric)
...
#'                kstest$transformed$p.value %>% as.numeric)
#'
#'     DtD.box$test = rbind(Dtd, transformed) %>% as.data.frame()
#'
#'     names(DtD.box$test) = nombre
#'   }
#'
#'   if (plot == F & ks.test == F) {
#'     return(DtD.box$x.box)
#'   } else if (ks.test == T) {
#'     return(DtD.box[-1])}
#'
#' }
